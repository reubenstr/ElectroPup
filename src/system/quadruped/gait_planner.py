...
class GaitPlanner:
    def __init__(
        ...
        stride_length: float,
        step_height: float,
        phase_offsets: Dict[LegName, float],
    ):
        self.gait = gait
    ...
    def foot_trajectory_bezier(self, phase: Phase, phase_time: float, stride_length_scale: float=None):
        """Return foot (d: distance, h: height) using Bezier swing and linear stance."""
        stride_length =  self.stride_length * stride_length_scale
        if phase == Phase.STANCE:
            ...
            control_points = np.array(
                [
                    [-stride_length / 2, 0],
                    [-stride_length, 0],
                    [-stride_length, self.step_height],
                    [0, self.step_height],
                    [stride_length, self.step_height],
                    [stride_length, 0],
                    [stride_length / 2, 0],
    ...
    def foot_trajectory_bezier_arc(self, phase: Phase, phase_time: float, stride_length_scale: float=None):
        ...
        if phase == Phase.STANCE:
            ...
            control_points = np.array(
                [
                    [-stride_length / 2, 0],
                    [-stride_length / 2, 0],
                    [-stride_length / 2, self.step_height],
                    [0, self.step_height],
                    [stride_length / 2, self.step_height],
                    [stride_length / 2, 0],
                    [stride_length / 2, 0],
    ...
    def foot_trajectory_sin(self, phase: Phase, phase_time: float, stride_length_scale: float=None):
        ...
        if phase == Phase.STANCE:
            ...
            d = phase_time * stride_length - stride_length / 2
            h = self.step_height * np.sin(np.pi * phase_time)
        return d, h

    def get_foot_position(self, leg: LegName, gait_time: float, stride_length_scale: float = 1) -> Point:
        phase, phase_time = self.get_leg_phase_time(leg, gait_time)
        if self.swing_pattern is SwingPattern.BEZIER:
            d, h = self.foot_trajectory_bezier(phase, phase_time, stride_length_scale)
        elif self.swing_pattern is SwingPattern.BEZIER_ARC:
            d, h = self.foot_trajectory_bezier_arc(phase, phase_time, stride_length_scale)
