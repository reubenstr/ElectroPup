...
from system.interfaces import MotionStates, Gaits
...
class Motion:
    def __init__(self):
        ...
        self.motion_state = MotionStates.BIAS_WALK
        self.target_motion_state: MotionStates = MotionStates.VECTOR_WALK
    ...
    def tick(
        ...
        if self.motion_state != MotionStates.POSE:
            ik_parameters = IKParameters()
    ...
    def increment_trajectory_index(self, motion_parameters: MotionParameters):
        if time() - self.tick_start_time > self.calculate_tick_delay(motion_parameters):
            ...
            direction = 1 if self.motion_state == MotionStates.TRANSITION else direction
    ...
    def calculate_tick_delay(self, motion_parameters: MotionParameters) -> float:
        ...
        """
        if self.motion_state == MotionStates.TRANSITION:
    ...
    def process_trajectory_state(
        ...
        if self.motion_state != self.target_motion_state:
            self.motion_state = MotionStates.TRANSITION
        ...
        # Process every loop:
        if self.motion_state in [MotionStates.VECTOR_WALK, MotionStates.BIAS_WALK]:
            self.trajectories = self.generate_trajectory(quad, motion_parameters, self.motion_state)
    ...
    def generate_trajectory(
        ...
        if motion_state == MotionStates.TRANSITION:
            if self.target_motion_state in [
                MotionStates.ROTATE,
                MotionStates.VECTOR_WALK,
                MotionStates.BIAS_WALK,
            ]:
                transition_type = TransitionType.STRAIGHT
            elif self.target_motion_state == MotionStates.POSE:
    ...
    def get_motion_state(self) -> MotionStates:
        return self.motion_state

    def get_target_motion_state(self) -> MotionStates:
        return self.target_motion_state

    def get_visual_rings(self) -> Trajectories:
        if self.motion_state == MotionStates.BIAS_WALK:
            return self.trajectory_planner.get_rings()
        else:
            return None

    def set_target_motion_state(self, state: MotionStates):
        self.target_motion_state = state

    def is_in_motion(self) -> bool:
        return self.motion_state != MotionStates.POSE
