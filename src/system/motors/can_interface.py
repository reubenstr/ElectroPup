    ...
    def req_pid(self, motor_id: int):
        ...
        if reply:
            ...
            if motor_id == reply_motor_id:
                ...
                return {"angle_kp": angle_kp, "angle_ki": angle_ki, "speed_kp": speed_kp, "speed_ki": speed_ki, "iq_kp": iq_kp, "iq_ki": iq_ki}
    ...
    def req_state_1(self, motor_id: int):
        ...
        if reply:
            ...
            if motor_id == reply_motor_id:
                ...
                if self.prints_enabled:
                    print(
                        f"[{self.tag }][M{reply_motor_id}] req_state_1 reply, temp.: {temperature}C, voltage: {voltage}V, UVP: {under_voltage_protection}, OVP: {over_voltage_protection}, OTP: {over_temperature_protection}, LIP:{lost_input_protection}"
                    )
                return {
    ...
    def req_state_2(self, motor_id: int):
        ...
        if reply:
            ...
            if motor_id == reply_motor_id:
                ...
                if self.prints_enabled:
                    print(
                        f"[{self.tag }][M{reply_motor_id}] req_state_2 reply, temp.: {temperature}C, watts: {watts}, motor speed: {motor_speed}, encoder position: {encoder_position}"
                    )
                return {"temperature": temperature, "watts": watts, "motor_speed": motor_speed, "encoder_position": encoder_position}
