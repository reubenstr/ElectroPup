...
export default function Plot3D({ hexData }: Plot3DProps) {
  ...
  const computePlotData = useMemo(() => {
    return (hexData: QuadData) => {
      const plotData: any[] = []

      if (hexData.plotSim && checkedValues.sim) {
        plotData.push(...generatePlotData(hexData.plotSim));
      }

      if (hexData.plotLive && checkedValues.live) {
        plotData.push(...generatePlotData(hexData.plotLive));
      }        
  ...
  useEffect(() => {
    if (hexData) {
      ...
      if (Date.now() - lastUpdateRef.current > max_plot_refresh_rate_ms) {
        ...
        setThrottledPlotData(computePlotData(hexData));
      }
    }
  }, [hexData, hold, computePlotData]);
  ...
  const generatePlotData = (plotData: PlotData): any[] => {
      ...
      if (plotData.body) { newPlotData.push({ x: plotData.body.x, y: plotData.body.y, z: plotData.body.z, type: 'scatter3d', mode: 'markers+lines', name: 'body', showlegend: false, marker: { color: 'blue' }, line: { shape: 'linear', width: 5, color: 'black' } }); }
