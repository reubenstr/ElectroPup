...
/* Web-only implementation. The native fallback lives in Plot3D.tsx; keeping
   the plotly imports out of that file means the Android/Hermes bundle never
   resolves the multi-MB dist, which is what OOMed the build. */
export default function Plot3D({ data }: Plot3DProps) {
  ...
  /* plotly.js touches `self`, so shim it before loading. The slim gl3d bundle
     goes through react-plotly's factory to avoid walking the full plotly
     source tree. */
  useEffect(() => {
