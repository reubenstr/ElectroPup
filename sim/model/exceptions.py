...
class JointOutOfBounds(Exception):
    ...
    def __init__(self, leg, joint, angle, lower_bound, upper_bound):
        ...
        self.message = f"Leg {leg} {joint} joint is out of bounds where angle {math.degrees(angle):0.2f} is outside of [{math.degrees(lower_bound):0.2f}, {math.degrees(upper_bound):0.2f}]!"
        super().__init__(self.message)